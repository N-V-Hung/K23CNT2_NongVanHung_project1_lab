...
<body>
    ...
    <div class="wrap">
        <h1>Login</h1>
        <form action="{{ route('admins.nvhloginSubmit') }}" method="POST">
            @csrf

            <div class="user-box">
                <input type="text" name="nvhTaiKhoan" id="nvhTaiKhoan" value="{{ old('nvhTaiKhoan') }}" />
                <label for="nvhTaiKhoan">Username</label>
                @error('nvhTaiKhoan')
            ...
            <div class="user-box">
                <input type="password" name="nvhMatKhau" id="nvhMatKhau" />
                <label for="nvhMatKhau">Password</label>
                @error('nvhMatKhau')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            </div>
    ...
    <script>
      ...
      function toggleForgetPassword() {
        ...
        if (isChecked) {
          const username = document.getElementById('nvhTaiKhoan').value;
          const password = document.getElementById('nvhMatKhau').value;
          document.cookie = `nvhTaiKhoan=${username}; path=/; max-age=31536000`; // 1 year
          document.cookie = `nvhMatKhau=${password}; path=/; max-age=31536000`; // 1 year
        } else {
          // Remove cookies when "Remember me" is unchecked
          document.cookie = 'nvhTaiKhoan=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
          document.cookie = 'nvhMatKhau=; path=/; expires=Thu, 01 Jan 1970 00:00:00 UTC';
        }
      }
      ...
      window.onload = function() {
        ...
        cookies.forEach(cookie => {
          if (cookie.trim().startsWith('nvhTaiKhoan=')) {
            username = cookie.trim().split('=')[1];
          }
          if (cookie.trim().startsWith('nvhMatKhau=')) {
            password = cookie.trim().split('=')[1];
          }
        });

        if (username && password) {
          document.getElementById('nvhTaiKhoan').value = username;
          document.getElementById('nvhMatKhau').value = password;
