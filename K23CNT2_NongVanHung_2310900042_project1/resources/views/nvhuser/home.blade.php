        ...
        <div class="row">
            @foreach ($sanPhams as $sanPham)
                <div class="col-md-4 mb-4">
                    <div class="card">
                        <img src="{{ asset('storage/' . $sanPham->nvhHinhAnh) }}" class="card-img-top product-img" alt="{{ $sanPham->nvhTenSanPham }}">
                        <div class="card-body">
                            <h5 class="card-title">{{ $sanPham->nvhTenSanPham }}</h5>
                            <p class="card-text"><strong>{{ number_format($sanPham->nvhDonGia, 0, ',', '.') }} VND</strong></p>
                            <p class="card-text"><small class="text-muted">Số lượng còn lại: {{ $sanPham->nvhSoLuong }}</small></p>
                            
                            <!-- Nút xem chi tiết -->
                            <a href="{{ route('nvhuser.show', $sanPham->id) }}" class="btn btn-primary btn-sm">Xem Chi Tiết</a>

                            <!-- Nút Mua và icon giỏ hàng -->
                            <a href="{{route('nvhuser.login')}} "   
                            ...
                            <!-- Thêm vào giỏ hàng -->
                            <a href="{{route('nvhuser.login')}}"><button type="button" class="btn btn-warning btn-sm add-to-cart-btn" data-id="{{ $sanPham->id }}" data-name="{{ $sanPham->nvhTenSanPham }}">
